...
const generateEmailContent = (data:IFormState) => {
  const stringData = Object.entries(data).reduce(
    (str, [key, val]) =>
      (str += `${CONTACT_MESSAGE_FIELDS[key as keyof IFormState]}: \n${val} \n \n`),
    ""
  );
  const htmlData = Object.entries(data).reduce((str, [key, val]) => {
    return (str += `<h3 class="form-heading" align="left">${CONTACT_MESSAGE_FIELDS[key as keyof IFormState]}</h3><p class="form-answer" align="left">${val}</p>`);
  }, "");
...
export async function POST(req: Request) {
  ...
  if (!data || !data.name || !data.email || !data.subject || !data.message || !data.phone || !data.termsConfirmed) {
    return new Response(JSON.stringify({ message: "Bad request" }), { status: 400 });
  }
