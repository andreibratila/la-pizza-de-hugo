...
export const ReactForm = () => {
  ...
  return (
        <form className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" onSubmit={handleSubmit}>
            ...
            {formContact.map(({ labelText, labelError, inputType, inputPlaceholder }, index) => {
                ...
                switch(labelError) {
                    ...
                    case 'phone':
                        ...
                        change = (e) => setPhone(e.target.value ? parseInt(e.target.value, 10) : null);
                ...
                return (
                    <div className="mb-4" key={index}>
                        <label className={`block text-gray-700 text-sm font-bold mb-2 ${error && 'text-red-700'}`}>
            ...
            <div className="mb-4">
                <label className={`block text-gray-700 text-sm font-bold mb-2 ${errors.message && 'text-red-700'}`} >
            ...
            <div className="mb-4 pl-1">
                <div className="flex items-start">
                    ...
                    <div className="ml-2 block text-sm">
                        <label htmlFor="terms" className={`text-gray-900 ${errors.termsConfirmed && 'text-red-700'}`}>
                            Acepto la <Link href="/politica-de-privacidad" className="font-bold">política de privacidad</Link> {errors.termsConfirmed && <span>{errors.termsConfirmed}</span>}
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between">
                <button
                    className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="submit" disabled={loading}>
                    {!loading ? 'Enviar': <Image src={Loading} className="w-6 h-auto" alt='Cargando'/>}
                </button>
            </div>
        </form>
    );
